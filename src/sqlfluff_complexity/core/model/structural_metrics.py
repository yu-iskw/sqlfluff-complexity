...
_STRUCTURAL_BY_ROOT: weakref.WeakKeyDictionary[BaseSegment, StructuralScanResult] = weakref.WeakKeyDictionary()
...
def direct_child_common_table_expressions(with_root: BaseSegment) -> tuple[BaseSegment, ...]:
    ...
    return tuple(
        seg for seg in getattr(with_root, "segments", ()) or () if getattr(seg, "type", "") == "common_table_expression"
    )
...
def _outer_sibling_table_reference_names(
    ...
    def visit(seg: BaseSegment) -> None:
        ...
        if st == "with_compound_statement":
            ...
            _record_outer_sibling_table_ref_if_applicable(seg, nested_frames, outer_sibling_names, collected)
        else:
