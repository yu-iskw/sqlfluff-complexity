...
def test_report_matches_lint_c110_derived_tables(tmp_path: Path) -> None:
    """Same thresholds should yield one CPX_C110 finding in report and lint."""
    sql = read_sql_fixture("ansi", "c110_derived_tables_two")
    sql_path = tmp_path / "model.sql"
    sql_path.write_text(sql, encoding="utf-8")
    cfg = tmp_path / ".sqlfluff"
    cfg_text = """
...
"""
    ...
    report = analyze_paths([sql_path], dialect="ansi", config_path=cfg)
    report_findings = [f for e in report.entries for f in e.findings if f.rule_id == "CPX_C110"]
    linted = lint_sql(sql, cfg_text, fname=str(sql_path))
    lint_v = rule_violations(linted, "CPX_C110")

    assert len(report_findings) == 1
    assert len(lint_v) == 1
    assert report_findings[0].threshold == 1
    assert "derived table count 2 exceeds max_derived_tables=1" in report_findings[0].message


def test_report_matches_lint_c113_aggregation_complexity(tmp_path: Path) -> None:
    """Same thresholds should yield one CPX_C113 finding in report and lint."""
...
"""
    ...
    report = analyze_paths([sql_path], dialect="ansi", config_path=cfg)
    report_findings = [f for e in report.entries for f in e.findings if f.rule_id == "CPX_C113"]
    linted = lint_sql(sql, cfg_text, fname=str(sql_path))
    lint_v = rule_violations(linted, "CPX_C113")

    assert len(report_findings) == 1
    assert len(lint_v) == 1
    assert report_findings[0].threshold == 7
    assert "aggregation complexity 8 exceeds max_aggregation_complexity=7" in report_findings[0].message
